...
const App: React.FC = () => {
  ...
  const results = useMemo((): CalculationResults => {
    const { views, bookmarks, coins: rawCoins, likes, danmaku, comments, shares, type, rankingType } = inputs;
    ...
    // Special case: "fake coin" rule (If no coins but other interactions exist, treat as 1 coin)
    let coins = rawCoins;
    if (coins === 0 && views > 0 && bookmarks > 0 && likes > 0) {
      coins = 1;
    }

    // Fix A: Repost coin compensation
    let fixA = 0;
    if (coins > 0) {
    ...
    // Fix B: TV/High view suppression
    let fixB = 0;
    ...
    // Fix C: Meme/High like suppression
    let fixC = 0;
    ...
    // Fix D: Comment anomaly suppression
    let fixD = 0;
    if (comments > 0) {
    ...
    return {
      ...
      scores: {
        views: viewScore,
        bookmarks: bookmarkScore,
        coins: coinScore,
        likes: likeScore,
        danmaku: danmakuScore,
    ...
    <div className="min-h-screen py-8 px-4 sm:px-6 lg:px-8 max-w-5xl mx-auto space-y-8">
      ...
      <section className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-6">
        ...
        <div className="pt-4 border-t border-gray-100">
           <span className="text-sm font-medium text-gray-700 block mb-2">视频类型 (Video Type)</span>
        ...
        <div className="lg:col-span-1 space-y-8">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <FactorDisplay label="fixA" value={results.fixA} description="硬币补偿" />
              <FactorDisplay label="fixB" value={results.fixB} description="播放抑制" />
              <FactorDisplay label="fixC" value={results.fixC} description="点赞抑制" />
              <FactorDisplay label="fixD" value={results.fixD} description="评论抑制" />
            </div>
          </div>
        </div>
